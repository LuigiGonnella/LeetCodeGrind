...
def stateful_dfs(num_nodes, edges):
    ...
    pre = {node: 0 for node in range(num_nodes)}
    time = 0
    def dfs(currNode):
        ...
        #TO TRACK FORWARD EDGES WE WOULD NEED TO TRACK DISCOVERY TIMES
        #IN (U,V) 
        #!IF TIME[V] > TIME[U] --> FORWARD EDGE
        #!IF TIME[U] > TIME[V] --> CROSS EDGE
        ...
        for node in graph[currNode]:
            if state[node] == 1:
                ...
                if pre[node] > pre[currNode]:
                    print(f" {currNode} --> {node} is a FORWARD EDGE EDGE")
                else:
                    print(f" {currNode} --> {node} is a CROSS EDGE EDGE")
