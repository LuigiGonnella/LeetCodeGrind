...
def stateful_dfs(num_nodes, edges):
    ...
    #!APPEND AS SOON AS A NODE FINISH THE DFS (POST-ORDER)
    #!IF WE REVERSE THIS ARRAY WE OBTAIN THE TOPOLOGICAL SORT --> topological_sort = inv_topological_sort[::-1]
    inv_topological_sort = []
    def dfs(currNode):
        ...
        if state[currNode] == 2: 
            # We hit a node we already fully explored -> CROSS OR FORWARD EDGE
            return 
        ...
        #process is finished
        state[currNode] = 2
        inv_topological_sort.append(currNode)
